...
visualizer = AnimatedPSOVisualizer(
    function, 
    bounds, 
    f"Quantum PSO - {function_name.capitalize()} Function",
    speed_factor=speed_factor
)

# Create QPSO optimizer
print(f"Starting Quantum PSO optimization on {function_name.capitalize()} function...")
print(f"Particles: {num_particles}, Max iterations: {max_iter}")
start_time = time.time()
qpso = QuantumPSO(function, num_particles, max_iter, bounds)

# Run optimization with visualization callback
results = qpso.optimize(callback=visualizer.callback)
